...
# v6 : Implementation d'une matrice RMS permettant de garder l'ordre des trials
...
FENETRE_TEMPS = 2.0 #Define the time window from which data is extracted. The time window is defined as : FENETRE_TEMPS/2 before the fork and FENETRE_TEMPS/2 after.
...
def main():
    ...
    for name in file_names_HFO:
        RMS_from_file = getDataFromFile(name)
        writeInFileOne(name, RMS_from_file)
...
def getDataFromFile(file_name):
    ...
    loop_start_time = start_time + PART_DURATION_START + 2*PART_DURATION_BODY + PART_DURATION_CHOICE  - int(FENETRE_TEMPS/2)
    loop_stop_time  = loop_start_time  + int(FENETRE_TEMPS) #PART_DURATION_CHOICE + PART_DURATION_FORK
...
def writeInFileGlobal(file_names, traitement_file_name, RMS_error, Perf, Perf_moy, Stdev, Stderr):
    ...
    traitement_file.write("Fenetre de temps :" + str(FENETRE_TEMPS) + "\n")
    traitement_file.write("\n")    
...
def writeInFileOne(file_name, RMS_from_file):
    ...
    f.write("Fenetre de temps :" + str(FENETRE_TEMPS) + "\n")
    f.write("\n") 
