...
def main():
    ...
    file_name = "/home/roche/MANIP/results/RESULTS_scenario_10_trial_1000_s_21-4-11-48.txt"
    ...
    for line in f:
        ...
        if k ==2:
            ...
            Theta1.append(float(dataList[3]))
            Theta2.append(float(dataList[4]))
...
#####################################################################################################
    ...
    if trial_number == 0:  
        i = 0
        j = len(Path_pos1)-1
    elif 0 < trial_number and trial_number <= int((PATH_DURATION - PART_DURATION_START - PART_DURATION_FORK)/ (2*PART_DURATION_BODY + PART_DURATION_CHOICE + PART_DURATION_FORK + PART_DURATION_REGRP)):
    ...
    ax1.set_xlabel('Position (steps)')
    ax1.set_ylabel('Time (s)')
    #ax1.axis([250, 550, int(Time[i])-1, int(Time[j])+1])

    ax2 = ax1.twiny()
    if file_name.find("_a_") != -1:
    ...
    if file_name.find("_s_") != -1:
        #ax2.axis([250, 550, int(Time[i])-1, int(Time[j])+1])
        pos1, = ax2.plot(Theta1[i:j],Time[i:j],'c-', alpha=0.5)
        pos2, = ax2.plot(Theta2[i:j],Time[i:j],'m-', alpha=0.5)
