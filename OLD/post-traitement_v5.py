...
# v5 : Implementation de l'ecriture des fichiers de resultats individuels (un par fichier)
...
def main():
    ...
    for name in file_names_HFO:
        (RMS_from_file, facteurDilatationFile) = getDataFromFile(name)
        writeInFileOne(name, RMS_from_file, Time, Path_pos1, Path_pos2, Curs_pos, facteurDilatationFile)
    ...
    writeInFileGlobal(file_names_HFO, traitement_file_name_HFO, RMS_error_HFO, Perf_HFO, Perf_moy_HFO, Stdev_HFO, Stderr_HFO)
    writeInFileGlobal(file_names_HFOP, traitement_file_name_HFOP, RMS_error_HFOP, Perf_HFOP, Perf_moy_HFOP, Stdev_HFOP, Stderr_HFOP)
...
def getDataFromFile(file_name):
    ...
    loop_start_time = start_time + PART_DURATION_START + 2*PART_DURATION_BODY + PART_DURATION_CHOICE - 1
    loop_stop_time  = loop_start_time + 2 #PART_DURATION_CHOICE + PART_DURATION_FORK
    ...
    return RMS_error, facteurDilatation
...
def writeInFileGlobal(file_names, traitement_file_name, RMS_error, Perf, Perf_moy, Stdev, Stderr):
...
def writeInFileOne(file_name, RMS_from_file, Time, Path_pos1, Path_pos2, Curs_pos, facteurDilatationFile):
    ...
    for i in range (0, max(len(RMS_from_file[0]), len(RMS_from_file[1]), len(RMS_from_file[2]))):
        ...
        f.write("\n")
    f.write("\n")
    f.write("Facteur de dilatation:\n")
    f.write(str(facteurDilatationFile)+ "\n")    
    f.write("\n\n")
    f.write("TIME\t\tPATH_POS1\t\tPATH_POS2\t\tCURS_POS\n")        
    for i in range (0, len(Curs_pos)):
        f.write(str(Time[i]) + "\t\t" + str(Path_pos1[i]) + "\t\t" + str(Path_pos2[i]) + "\t\t" + str(Curs_pos[i]) + "\n")       
